...
const init = async() => {
    ...
    await db.run('create table if not exists vagas(id INTEGER PRIMARY KEY, categoria INETEGER, titulo TEXT, descricao TEXT);')
    //const categoria = 'Marketing Team'
    //await db.run(`insert into categorias(categoria) values('${categoria}')`)
    const vaga = 'Social Media (San Francisco)'
    const descricao = 'Vaga para Fullsatck developer que fez o fullstack lab'
    //await db.run(`insert into vagas(categoria ,titulo, descricao) values(2, '${vaga}', '${descricao}' )`)
}
init()
app.listen(port, (err) => {
    if(err){
        console.log('Não foi possivel executar o servidor do Jobify')
    } else {
